...
<html>
  ...
  <body>
    ...
    $query = "UPDATE tbl_books SET isbn10 = ?, title = ?, publisher_id = ?, year = ?, pages = ?, back_page = ?, category_id = ?, translated = ?, translator_id = ?, eidos_grafis_id = ?, copies_standard = ?, copies_avail = ?, in_stock = ? 
     WHERE isbn = ? RETURNING *";
    
    // Author deletes from table and than insert selected values.
    $query2 = "DELETE FROM books_authors WHERE isbn = ?";
    $query3 = "INSERT INTO books_authors (isbn, author_id) VALUES (?, ?)";
    
    $params = array($isbn10, $title, $publisher_id, $year, $pages, $back_page, $category_id, $translated, $translator_id, $eidos_grafis_id, $copies_standard, $copies_avail, $in_stock, $isbn);
    ...
    try{
        ...
        foreach ($author_id as $a){
            ...
            $params3 = array($isbn, $a);
            $count_author += $result3 -> execute($params3);
          }
        $con->commit();
    }
    ...
    if (sizeof($book )>0){
      foreach ($book as $b){
        ...
        //   $params3 = array($b['isbn'], $author_id[0]);
          ...
          echo '<div id="myModal" class="modal">';
            echo"<div class='modal-content'>";
              echo "<div class='modal-header'>";
                ...
                echo "<h2>Επιτυχής Ενημέρωση Βιβλίου με ISBN: ".$b['isbn']."</h2>";
                if ($count_author == 1){
                  echo "<h3>Επιτυχής Ενημέρωση Συγγραφέα: ";
                ...
                elseif ($count_author > 1){
                  ...
                  echo "<h3>Επιτυχής Ενημέρωση Συγγραφέων: ";
              ...
              echo "</div>";
                ...
                echo "</div>";
              ...
              catch (PDOException $e)
                  {
                    ...
                    echo "<div class='modal-header'>";
                      echo '<span class="close">&times;</span>';
                      echo "<h2>Δεν έγινε ενημέρωση του βιβλίου.</h2>";
                    echo "</div>";
                    echo "<div class='modal-body'>";
